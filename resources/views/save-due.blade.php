...
<body class="antialiased bg-zinc-900">
    ...
    <div class="absolute w-full flex justify-end mt-6">
        ...
        <div id="toast-danger"
            ...
            <div id="error_toasttext" class="ms-3 text-sm font-normal">Erro.</div>
            <button onclick="$('#toast-danger').addClass('hidden')" type="button"
                class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700"
    ...
    <div class="bg-zinc-900 flex justify-center items-center w-12/12 h-screen">
        <form id="formEnvio" onsubmit="return sendForm(event);" method="post" enctype="multipart/form-data"
            class="flex flex-col justify-center items-center gap-4">
            @csrf
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" htmlFor="file_input">
                    Faça o envio do JSON
                </label>
                <input
                    ...
                    name="json" id="json" type="file" accept=".json" />
            </div>
            <button
...
<script>
    function sendForm(e) {
        e.preventDefault();
        var formData = new FormData($('#formEnvio')[0]);
